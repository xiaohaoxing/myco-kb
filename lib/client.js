...
window.__ModuleLoader__?.load?.({
  ...
  factory: (require) => {
    ...
    function CloudSection({ myco, t }) {
      ...
      // 订阅管理（opt-in）：每个挂载的云端包显示订阅状态 + 切换
      for (const m of data.mounts ?? []) {
        if (!m.resolved) continue
        const toggle = () => {
          setBusy(true)
          Promise.resolve(myco.cloudSubscribe(m.spec.replace('cloud:', ''), !m.subscribed))
            .then(refresh)
            .catch(() => {})
            .finally(() => setBusy(false))
        }
        children.push(React.createElement('div', {
          key: m.spec + '-sub',
          style: { fontSize: '12px', marginTop: '6px', display: 'flex', alignItems: 'center', gap: '8px' },
        },
          React.createElement('span', null,
            `${m.spec}  ${m.subscribed ? `🔔 ${t('cloud.subscribed')}` : `○ ${t('cloud.unsubscribe')}（未订阅，不自动同步）`}`),
          button(m.subscribed ? t('cloud.unsubscribe') : t('cloud.subscribe'), toggle, busy),
        ))
      }
      return React.createElement('div', { style: sectionStyle() }, children)
    }
