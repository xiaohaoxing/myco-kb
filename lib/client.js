...
window.__ModuleLoader__?.load?.({
  ...
  factory: (require) => {
    ...
    const NS = 'myco-kb'
    const inject = ['slots', 'remote', 'remote.myco', 'workspaces', 'locale']
    ...
    const dictionaries = {
      ...
      en: {
        ...
        'cloud.empty': 'No cloud roots configured (sync lands in v0.3)',
    ...
    function CloudSection({ myco, t }) {
      ...
      if (roots.length === 0) {
        children.push(React.createElement('div', { style: { fontSize: '12px' } }, t('cloud.empty')))
      } else {
        children.push(React.createElement('div', { style: { fontSize: '12px', marginBottom: '8px' } },
          `${t('cloud.roots')}: ${roots.map(([k, v]) => `${k} → ${v}`).join('; ')}`))
      }
      for (const m of data.mounts ?? []) {
        const note = m.resolved
          ? `${m.path}`
          : `${t('cloud.unresolved')} — ${m.reason ?? ''}`
    ...
    // ---------- 控制台面板 ----------
    function MycoConsolePanel({ myco, workspaces, t }) {
