...
export class MycoRemoteService extends TypertRemoteService {
  ...
  cloudSync(name) {
    return name === undefined ? this.core.syncAll() : this.core.syncPackage(name)
  }

  cloudAdd(name, url, options) {
    this.core.cloudAdd(name, url, options ?? {})
    return this.core.cloudList()
  }

  cloudRemove(name) {
    this.core.cloudRemove(name)
    return this.core.cloudList()
  }
