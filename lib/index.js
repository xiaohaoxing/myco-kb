...
import { registerTools, registerAgentAssembly } from './tools.js'
    ...
    try {
      switch (cmd) {
        ...
        case 'assemble': {
          ...
          if (!goal) {
            sendJson(res, 400, { error: '需要 goal 查询参数' })
            return
          }
          sendJson(res, 200, myco.assemble(goal))
          return
        }
        case 'assemble/last': {
          sendJson(res, 200, myco.lastAssemble())
          return
        }
        // ---- v0.5 知识更新流 ----
        case 'scan': {
        ...
        case 'telemetry': {
          if (req.method === 'GET' || req.method === 'HEAD') {
            sendJson(res, 200, myco.getTelemetry())
            return
          }
          const raw = await readBody(req)
          const { enabled, url, intervalHours } = raw ? JSON.parse(raw) : {}
          sendJson(res, 200, myco.setTelemetry({ enabled, url, intervalHours }))
          return
        }
        case 'telemetry/now': {
          sendJson(res, 200, await myco.sendTelemetry())
          return
        }
        // ---- meta（客户端设置页：开发者链接 + 版本）----
        case 'meta': {
          const links = {
            source: config.links?.source ?? '',
            docs: config.links?.docs ?? '',
            site: config.links?.site ?? '',
          }
          sendJson(res, 200, { version: myco.collectTelemetry().version, links, telemetryUrl: myco.getTelemetry().url })
          return
        }
        // ---- v0.5.3 subagent 起草 ----
        case 'draft': {
...
export function apply(ctx, config = {}) {
  ...
  const myco = new Myco({ dataDir, telemetry: config.telemetry })
  ...
  // v0.4.2 把按任务动态装配接进 agent 生命周期（opt-in：config.assemble.auto）
  ctx.effect(() => {
    const r = registerAgentAssembly(ctx, myco, config)
    return r?.ok ? r.off : undefined
  }, 'myco-kb: agent assembly')
