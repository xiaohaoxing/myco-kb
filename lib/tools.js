...
// presentCall 必须返回 dsh-tools 的 render-intent 视图，且以 `card` 字段标识卡型
// （参考实现在 dsh-tool-jobs：{ card: 'generic', title, kind, rawInput }）。
// 缺 `card` 会被宿主 viewFor 当作未知卡型回退到通用卡片——能兜底但不是契约形状。
