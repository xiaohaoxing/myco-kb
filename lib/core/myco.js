...
export class Myco {
  ...
  getTelemetry() {
    ...
    return {
      ...
      // url 优先用户显式配置（config.json），否则回退到插件默认（开发者统计端点）
      url: t.url || this.telemetryDefaultUrl || '',
      intervalHours: t.intervalHours ?? 24,
      lastSentAt: t.lastSentAt ?? null,
      instanceId: t.instanceId ?? null,
    }
  }

  setTelemetry({ enabled, url, intervalHours } = {}) {
    const t = this.config.telemetry ?? {}
  ...
  async sendTelemetry() {
    ...
    const payload = this.collectTelemetry()
    try {
