...
> MyCo-KB（真菌之库 · 我的公司）是面向 **DeepSeek Harness** 的知识库管理系统插件。它把「知识库」从一堆文件升级为可挂载、可检索、可演进、可传播的系统：知识包统一管理、组合配置按需激活、生命周期自动演进、跨包检索即问即答、云端 git 同步、契约驱动的知识更新流。
...
- tag 命中 ×3（全库命名空间 tag 如 `chaoheti` 为停用词）；
- 文件名命中 ×2；
- 全文命中 ×1；
- 证据页带 `[证据]` 标记，方便按可信度取舍。
